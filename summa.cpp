...
#include <omp.h>
using namespace std;
#define WORK_LOAD 10 //the amount of row+col 1 thread work with

// timing naive sequential, parallel, and summa parallel

// Part 1 (loop order is i,j,k)
...
// Part 2 (A beeter way to multiply matrices in k,i,j order)
...
void *thread_mult(void *arg)
{
    ...
    //k is to loop through all the work a thread has to do
    for (int k = 0; k < WORK_LOAD; k++)
    {
        //start broad cast A and B
        ...
        //critical section update C based on col and row
...
int **parallel_summa(int **data_ptr, int **data_ptr2, int n)
{
    ...
    //number of threads = n/WORK_LOAD each thread has to do
...
void *pipeline_mult(void *arg)
{
    ...
    */
    ...
    int id = data->id;
    //col and row to broadcast matrix A and B 
